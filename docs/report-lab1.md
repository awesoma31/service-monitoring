...
| `config` | хеширование паролей, OpenAPI, включение планировщика |
...
Скрипт `scripts/demo.sh` последовательно выполняет основной сценарий: создание
пользователя и проекта, конфликтные и некорректные запросы, обе формы пагинации и полный
цикл инцидента от падения монитора до автоматического закрытия.
...
| Тесты веб-слоя (`@WebMvcTest`) | формат ошибок и коды ответов |
...
(JaCoCo). Текущие показатели: 115 тестов, покрытие строк 97,6%, ветвлений 88,1%.
...
| Покрытие тестами не ниже 70% | JaCoCo в задаче `check`; фактически 97,6% |
