...
| Участник | пользователь, добавленный в проект; хранится дата вступления |
...
| `config` | OpenAPI, включение планировщика |
...
Схема создаётся только миграциями Liquibase: 15 наборов изменений (changeset) в формате
...
| Создание проекта (`ProjectService.create`) | создание проекта, добавление владельца в `project_members` | сбой между двумя вставками оставил бы проект, владелец которого не числится среди участников; через API такое состояние не обнаружить и не исправить |
...
пользователей и проекта, добавление участника, конфликтные и некорректные запросы, обе
формы пагинации и полный цикл инцидента от падения монитора до автоматического закрытия.
...
(JaCoCo). Текущие показатели: 110 тестов, покрытие строк 97,7%, ветвлений 88,2%.
...
| Покрытие тестами не ниже 70% | JaCoCo в задаче `check`; фактически 97,7% |
