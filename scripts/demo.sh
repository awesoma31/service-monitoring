...
state() { curl -s "$API/monitors/$1" | jq -r .currentState; }
...
R=$(call POST /users "{\"email\":\"member-$RUN@demo.io\",\"password\":\"secret123\",\"fullName\":\"Demo Member\"}")
MEMBER_ID=$(body "$R" | jq .id)
show POST "/projects/$PROJECT_ID/members" "{\"userId\":$MEMBER_ID}"
...
curl -s "$API/monitors/$BROKEN/results?size=2" | jq -c '{rows: (.content | map(.result)), hasTotal: has("totalElements"), last}'
