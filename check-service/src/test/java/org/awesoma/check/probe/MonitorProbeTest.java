...
class MonitorProbeTest {
    ...
    private final MonitorProbe probe = new MonitorProbe(WebClient.builder());
    ...
    void reportsSuccessWhenTheStatusMatchesWhatIsExpected() {
        ProbeOutcome outcome = probe.probe(target(baseUrl + "/ok", 200, 2000)).block();
    ...
    void reportsBadStatusWhenTheAnswerDiffersFromTheExpectedOne() {
        ProbeOutcome outcome = probe.probe(target(baseUrl + "/teapot", 200, 2000)).block();
    ...
    void anExpectedNonSuccessStatusStillCountsAsSuccess() {
        ProbeOutcome outcome = probe.probe(target(baseUrl + "/teapot", 418, 2000)).block();
    ...
    void reportsTimeoutWhenTheServerAnswersTooLate() {
        ProbeOutcome outcome = probe.probe(target(baseUrl + "/slow", 200, 300)).block();

        assertThat(outcome.result()).isEqualTo(CheckResultType.TIMEOUT);
        assertThat(outcome.httpStatus()).isNull();
        assertThat(outcome.errorMessage()).contains("300 ms");
    }
    ...
    void reportsConnectionErrorWhenNothingIsListening() throws IOException {
        ...
        ProbeOutcome outcome =
                probe.probe(target("http://127.0.0.1:" + closedPort + "/", 200, 1000)).block();
    ...
    @Test
    void anUnusableUrlIsAConnectionErrorRatherThanAnException() {
        ProbeOutcome outcome = probe.probe(target("http://", 200, 1000)).block();
